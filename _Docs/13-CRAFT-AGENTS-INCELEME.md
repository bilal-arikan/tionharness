# external-agent-oss Release İncelemesi → SwarmGo Çıkarımları
...
> Tarih: 2026-06-17 · Yöntem: tüm release gövdeleri çekildi, paralel LLM ile feature/fix çıkarımı, ardından SwarmGo kod tabanına karşı doğrulama.

Bu doküman iki bölümden oluşur:
1. **SwarmGo için önceliklendirilmiş eylem listesi** — kod tabanına karşı doğrulanmış (EXISTS/MISSING).
2. **Sürüm-sürüm tam fix & feature listesi** (referans/appendix).

---

## 1. SwarmGo İçin Önceliklendirilmiş Eylem Listesi

Her madde SwarmGo kaynak koduna karşı kontrol edildi. **Durum** = şu anki SwarmGo durumu.

### 🔴 P0 — Doğrulanmış gerçek boşluklar (yüksek etki)

| # | Konu | external-agent kaynağı | SwarmGo durumu | Öneri |
|---|------|----------------------|----------------|-------|
| 1 | **Fable 5 / adaptive-thinking** | v0.10.3 | ⚠️ `thinkingBudgetForLevel` sabit (low/med/high→budget), model-sınıfı farkındalığı **YOK** (`toolloop.go:17`) | Fable/Mythos 5 sınıfı modeller `thinking: disabled` reddediyor — API 400 verir. Model registry'ye `requiresAdaptiveThinking` bayrağı ekle; bu modellerde "off" → düşük-eforlu adaptive'e map et. Opus/Sonnet/Haiku değişmez. **SwarmGo şu an opus-4-8 ile çalışıyor; Fable 5 eklenince kritik.** |
| 2 | **Tool çıktısı boyut sınırı** | v0.4.4 (100K/200K cap), v0.9.3 | ❌ **MISSING** — `res.Content` → `TurnStep.Output` → JSONL'e sınırsız akıyor (`toolloop.go:254`) | Persistence ve modele gönderim öncesi char/byte cap uygula (örn. 100K trunc + "…[truncated]"). Kontrolsüz tool çıktısı OOM + bağlam taşması + JSONL şişmesi yapar. |
| 3 | **http_get SSRF koruması** | v0.3.2, v0.5.0 | ⚠️ Boyut cap **VAR** (64KB, `builtin_http.go:14`) ama SSRF koruması **YOK** | localhost/127.0.0.1, özel ağlar (10/8, 172.16/12, 192.168/16), link-local 169.254/16 ve cloud metadata IP'lerini engelle. Aksi halde ajan iç ağ/metadata uçlarına erişebilir. |
| 4 | **MCP şema sağlamlığı** | v0.7.3, v0.7.5, v0.7.12 | ❌ `InputSchema` ham geçiyor; `$schema` strip yok, `additionalProperties` korunmuyor, oneOf/anyOf/allOf işlenmiyor (`internal/mcp`) | Dış MCP tool şemalarını provider'a göndermeden önce normalize et: `$schema` alanını kaldır (Anthropic 400 verir), `additionalProperties`'i koru, union keyword'leri ele al. Aksi halde tool parametreleri sessizce kaybolur veya çağrı reddedilir. |
| 5 | **MCP tool metadata sızıntısı** | v0.7.7, v0.8.13 | ⚠️ Şu an SwarmGo iç metadata enjekte etmiyor (temiz) — ama Craft tool çağrı kuralı `_displayName`/`_intent` zorunlu kılıyor | Eğer ileride UI için tool şemasına meta alan eklenirse, provider'a göndermeden **boundary'de strip et**. Şimdilik not; tasarımı bozma. |

### 🟠 P1 — Çok-ajan mimarisine doğrudan uyan eksikler

| # | Konu | Kaynak | SwarmGo durumu | Öneri |
|---|------|--------|----------------|-------|
| 6 | **Ajanlar-arası mesajlaşma** | v0.8.8 `send_agent_message` | ❌ **MISSING** | Çok-ajan orkestrasyonun temel primitifi. Bir oturumun/ajanın başka aktif oturuma mesaj göndermesini sağlayan built-in tool ekle. Flow motoruyla (`orchestration`) birleşince güçlü. |
| 7 | **Oturum öz-yönetim araçları** | v0.8.3 | ⚠️ Kısmî — API var, ajan-tool yok | Ajana `set_session_labels` / `set_session_status` / `get_session_info` / `list_sessions` built-in tool'ları ver → kendini-kapatan otomasyon akışları (görev bitince status=done → trigger). |
| 8 | **Hooks / koşullu otomasyon** | v0.4.3, v0.7.7, v0.7.5 | ⚠️ `events` bus + `scheduler` var; hook/condition/webhook yok | (a) Command + prompt hook'ları (olay → shell/prompt enjeksiyonu), rate limiter + zorla-sonlandırma. (b) Otomasyon koşulları (time/state/label gate). (c) Webhook action (exponential backoff retry). SwarmGo'nun schedule sistemiyle birebir örtüşür. |
...
- **Shell sandbox escape** (v0.5.0): `find -exec` ve benzeri escape vector'lerini engelle (SwarmGo shell varsayılan kapalı ama açıkken geçerli).
...
- **Path traversal** (v0.3.2): `sessionId`/`rel` ile attachment/upload yazımında — SwarmGo `sandbox.go` Resolve koruması **VAR**; upload/artifact yollarında da aynı sanitize uygulandığını doğrula.
...
- **Render blokları**: Mermaid native (v0.3.0), HTML/PDF/image/markdown preview (v0.4.6, v0.9.6), datatable/spreadsheet + `transform_data` (v0.4.2). SwarmGo'nun artifact sistemine eklenebilir.
...
external-agent-oss'un yolculuğu SwarmGo için bir **yol haritası önizlemesi**: tek-provider → multi-provider olgunlaşma, Electron IPC → WebSocket RPC + headless server, ve giderek artan **otomasyon/messaging/orkestrasyon** katmanları. SwarmGo'nun mevcut mimarisi (file-based store, iki-parçalı sistem prompt, provider soyutlaması, events bus, scheduler) bu yörüngeyle **uyumlu**; en yüksek getirili adımlar P0–P1 tablolarındaki doğrulanmış boşluklar.
