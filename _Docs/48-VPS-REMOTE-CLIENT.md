...
> **Amaç:** SwarmGo backend'ini bir VPS'te (7/24 Linux) çalıştırıp, telefondan
...
Kullanıcı SwarmGo'ya telefondan erişmek istiyor. İki mimari değerlendirildi:
...
Karar: **VPS modeli.** Çekirdek SwarmGo'ya neredeyse dokunmadan, asıl iş
...
```
  ...
  ┌───────────────────────┐        ┌──────────────────────────────┐
  │ WebView → https://vps │══VPN══▶│ SwarmGo binary (DEĞİŞMEDEN)   │
...
`workspaceler ve temel dosyalar VPS'te durur` = SwarmGo'nun bugünkü **dosya-tabanlı
...
- SwarmGo yine **`127.0.0.1` ya da tailnet arayüzüne** bind eder → **public'e hiç
  çıkmaz.** "auth-yok/CORS-wildcard" felsefesi olduğu gibi korunur, **SwarmGo'ya
  auth kodu eklemeye gerek kalmaz.**
- Tek kullanıcı + kişisel kullanım için en pratik, en az efor.
- Bind adresi: `SWARMGO_ADDR=<tailscale-ip>:8080` (veya `127.0.0.1` + Tailscale
...
- nginx/caddy ile TLS + token/mTLS auth, `0.0.0.0` bind arkada. SwarmGo'ya bir
...
> **Not:** VPN seçimi, SwarmGo kodunda auth değişikliği gerektirmeyen tek yol.
...
|-----|-----|-------------------|
| **F0** | VPS'e SwarmGo kur (`go build` linux/amd64) + claude-cli login (izole config home) + Tailscale kur, tailnet bind | Yok (deploy) |
