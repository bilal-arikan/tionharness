...
| [75-YAYIN-SURECI.md](75-YAYIN-SURECI.md) | **CI hattı UYGULANDI (2026-08-27)** — Etiket→test→derleme→yayın akışı (`.gitea/workflows/release.yml`), `latest.json` şeması, `RELEASE_HOST`/`RELEASE_USER`/`RELEASE_PATH`/`RELEASE_SSH_KEY`/`FEED_BASE` secret'ları, VPS yokken yayın adımının atlanması ve `deploy/release-host/` ile yerel Docker testi |
