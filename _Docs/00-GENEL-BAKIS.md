...
- **Provider (Sağlayıcı):** LLM uç noktası soyutlaması (9 kind: `anthropic`, `claude-cli`, `codex-cli`, `minimax`, `minimax-anthropic`, `openrouter`, `zai`, `deepseek`, `deepseek-anthropic`).
...
| [71-SAGLAYICI-ORNEKLERI-PLANI.md](71-SAGLAYICI-ORNEKLERI-PLANI.md) | **Faz 0-5 BİTTİ (2026-08-18)** — Sağlayıcı taslak→örnek (ProviderKind→ProviderInstance) modeli: aynı kind'dan birden fazla, farklı token/config taşıyan uygulama-geneli sağlayıcı örneği; `Manifest.Fields` alan şeması ile generic ayar formu (`/api/provider-kinds` + `/api/providers` CRUD + `ProviderInstanceList`/`ProviderInstanceForm`); `providers.json` şifreli örnek deposu (K2); varsayılan örnek id'leri = kind id'leri hilesiyle **sıfır veri migrasyonu**; `Agent.ProviderInstanceID` alanı + `Agent.Provider`'ın türetilmiş **kind id'sine** dönüşmesi (K3 — fiyat/context/usage okuyucuları dokunulmadan çalışır); `== "claude-cli"` davranış anahtarları → `Manifest.Transport`; CLI config evi geri uyumlu (boş `configDir` → workspace claude-home, K1); ajan formu örnek-bazlı seçim (`ProviderInstanceModelSelect`); eski typed `settings.Settings` alanları (Minimax/OpenRouter/Z.ai/DeepSeek/CustomProviders) DTO/Patch'ten kaldırıldı, `InstanceCatalog()` örnek-bazlı (aynı kind'ın iki örneği ayrı görünür) |
