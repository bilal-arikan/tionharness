...
| [66-VIEW-KATMANI.md](66-VIEW-KATMANI.md) | **TASLAK** — View (projeksiyon) katmanı: uzun session / çok-node'lu flow run / yüzlerce kartlı board / workspace durumunun bağlam-ucuz özeti. Deterministik L0/L1 + opsiyonel L2 incremental fold, `tiny/card/full` bütçe tier'ları, üç dağıtım kanalı (pull aracı / volatile push / handle) ve **aynı çıktıyı gösteren `ViewPanel` UI'ı** |
