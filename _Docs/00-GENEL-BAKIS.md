...
| [68-OZET-HARITASI.md](68-OZET-HARITASI.md) | **Faz 1-3 canlı** — Özet Haritası / Workspace Explorer: View katmanı üstünde kök düğümden tıkladıkça bir katman açılan semantic-zoom drill-down. Backend: `agent`/`budget`/`tools`/`category` Kind'leri + deterministik projeksiyonlar (LLM yok), `Children(ref, lens)` + `GET /children`, ajan **`expand` aracı**. Frontend: NavRail "Harita" ekranı (`features/explorer`), React Flow lazy-expand, gömülü özet paneli, semantic zoom, lens, canlı SSE, **URL deep-link + harita-içi arama + DOI soldurma**. Opsiyonel/ertelendi: MCP resource tree + sigma.js (büyük-workspace) |
