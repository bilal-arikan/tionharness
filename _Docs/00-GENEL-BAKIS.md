...
| [71-SAGLAYICI-ORNEKLERI-PLANI.md](71-SAGLAYICI-ORNEKLERI-PLANI.md) | **Plan (kod yok)** — Sağlayıcı taslak→örnek (ProviderKind→ProviderInstance) modeli: aynı kind'dan birden fazla, farklı token/config taşıyan uygulama-geneli sağlayıcı örneği; `Manifest.Fields` alan şeması ile generic ayar formu; `providers.json` şifreli örnek deposu; varsayılan örnek id'leri = kind id'leri hilesiyle **sıfır veri migrasyonu**; kind-anahtarlı fiyat/context çağrılarının (`PriceFor`, `ContextWindowFor`, `MaxOutputFor`, `CoolingWaste`) ve `== "claude-cli"` davranış anahtarlarının `KindOf`/`Manifest.Transport` ile düzeltilmesi. Faz 0 kapısı: codex-cli merge'ü (Doc 70) |
