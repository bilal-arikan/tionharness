...
| [71-SAGLAYICI-ORNEKLERI-PLANI.md](71-SAGLAYICI-ORNEKLERI-PLANI.md) | **Plan (kod yok)** — Sağlayıcı taslak→örnek (ProviderKind→ProviderInstance) modeli: aynı kind'dan birden fazla, farklı token/config taşıyan uygulama-geneli sağlayıcı örneği; `Manifest.Fields` alan şeması ile generic ayar formu; `providers.json` şifreli örnek deposu (K2); varsayılan örnek id'leri = kind id'leri hilesiyle **sıfır veri migrasyonu**; yeni `Agent.ProviderInstanceID` alanı + `Agent.Provider`'ın türetilmiş **kind id'sine** dönüşmesi (K3 — fiyat/context/usage okuyucuları dokunulmadan çalışır); `== "claude-cli"` davranış anahtarları → `Manifest.Transport`; CLI config evi geri uyumlu (boş `configDir` → workspace claude-home, K1). Faz 0 kapısı: codex-cli merge'ü (Doc 70) |
