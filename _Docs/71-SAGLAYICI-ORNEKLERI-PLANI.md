...
> **Durum: PLAN (kod yok).** Kapsam: bugünkü "provider = sabit kind id" modelini
...
**Öneri:** `settings.json` içinde `providerInstances: []` olarak **değil**, ayrı
...
- `Agent.Provider` değerlerine **hiç dokunulmaz** — "anthropic" hâlâ geçerli bir
  örnek id'sidir.
...
### 4.1 Kind-anahtarlı fonksiyonlara örnek id'si geçen çağrılar

Bu fonksiyonlar `provider` parametresini **kind** sanıyor. `PRV3` geçince
sessizce 0/false dönerler (fiyat yok, context window yok) — **sessiz bozulma**.
Her çağrı önce `KindOf(id)` ile çözülmeli:
...
Çözüm: `Registry.KindOf(id string) string` + bu 5 dosyada çözümleme.
Registry'ye erişimi olmayan `billing`/`conversation` için: çağrı **sınırında**
(runtime/api) kind'a çevirip parametre olarak geçir — global resolver hook'u
(gizli tekil state) **kullanma**.
...
### 4.4 CLI config evi: workspace mi, örnek mi?
...
**Öneri (geri uyumlu):**
- Örneğin `config["configDir"]` **boşsa** → bugünkü davranış (workspace claude-home).
- **Doluysa** → örneğin kendi evi kullanılır (`<dataDir>/provider-homes/<PRV…>/`).

Böylece mevcut kurulumlar aynen çalışır; iki hesap isteyen kullanıcı ikinci
örneğe ayrı bir ev verir. Aynısı `CODEX_HOME` için geçerli.
**→ Onay bekleyen karar (bkz. §8-K1).**
...
| `features/agents/AgentSettingsForm.tsx` | Provider seçici artık örnek listesi (label + kind rozeti + ⚠ yapılandırılmamış). Model seçici örneğin kind manifest'i + örnek `models` override'ı |
...
| **1** | `FieldSpec` + `Manifest.Fields/Transport`; her `kind_*.go` kendi alanlarını ilan eder; `ProviderInstance` modeli + `providers.json` store + **boot migrasyonu**. Registry hâlâ eski yoldan okur (ikili yazım) | Migrasyon testi: eski settings.json → beklenen örnek seti; davranış birebir aynı | ~1 gün |
| **2** | `Registry` örnek-tabanlı: `SetInstances()`, `resolve()` örnekten, `KindOf()`; §4.1'deki 5 çağrı + §4.2'deki 5 karşılaştırma düzeltilir; `openai-compat`/`anthropic-compat` kind'ları → `CustomSpec` yolu düşer | Aynı kind'dan 2 örnek testi (2 anthropic, farklı key) yeşil; fiyatlama örnek id ile doğru | ~1 gün |
| **3** | API (`/api/provider-kinds`, CRUD, `/test`) + `ProvidersPanel` generic form | UI'dan 2. bir Anthropic örneği yaratılıp ajana bağlanabiliyor | ~1.5 gün |
...
- **K1 — CLI config evi.** §4.4'teki geri uyumlu öneri (boş `configDir` →
  workspace claude-home, dolu → örneğe özel ev) kabul mü? Alternatif: config evi
  **her zaman** örneğe taşınır (daha temiz model, ama mevcut workspace login'leri
  bir kez taşınmalı).
- **K2 — Depolama.** Ayrı `providers.json` (önerilen) mi, `settings.json` içinde
  `providerInstances[]` mi?
- **K3 — Alan adı.** `Agent.Provider` alan adı korunup **semantiği** "örnek id"
  olsun (migrasyon sıfır) — yoksa `Agent.ProviderInstanceID` diye yeni alan mı
  açılsın (daha okunur, ama tüm şablon/pack/ingest yollarında geçiş işi)?
  Öneri: **korunsun**.
...
| Kind-anahtarlı fonksiyonların sessiz 0 dönmesi (fiyat/window) | §4.1 envanteri + `KindOf` bilinmeyen id'de **panik değil hata**; birim test her çağrı sınırında |
...
- `internal/billing`: örnek id ile fiyatlama (kind çözümü sonrası) — mevcut
