...
schedules, flows, hooks, mcp_servers. Detay: `_Docs/02-VERI-MODELI.md`.

## Araç ailesi

| Alan | Araçlar | Notlar |
|------|---------|--------|
| **Agents** | `create_agent` / `update_agent` / `delete_agent` / `list_agents` | create canlı `Start`, delete `Stop`; delete provenance-guard'lı |
| **Ajanlar-arası** | `send_agent_message` | fire-and-forget inbox teslimi + `Wake` (async; `call_agent` senkron) |
| **Spawn** | `spawn_session` | yeni bağımsız oturum, beklemeden; tur başına `SpawnMaxPerTurn` limiti. Bkz. `22-SPAWN-SESSION.md` |
...
| **Skills** | `create_skill` / `delete_skill` | yalnız **workspace-tier**; global/bundled skill store tarafından korumalı. `use_skill` ile sonraki turdan yüklenebilir |
| **App-ayarları** | `get_settings` / `update_settings` | aşağıdaki ayarlar köprüsü; secret alanları maskeli |

> Kaynaklar: `internal/tools/builtin_{agentmgmt,flowmgmt,schedulemgmt,taskmgmt,hookmgmt,mcpmgmt,artifactmgmt,memory_add,logs,secretmgmt,skillmgmt,settings}.go`,
> `builtin_agentmsg.go`, `builtin_spawn.go`. Guard testleri: `builtin_controlgaps_test.go`,
> `builtin_taskmgmt_test.go`, `builtin_settings_test.go`.
