...
| **Oturum yönetimi (workspace-scoped)** | `list_sessions` / `update_session` / `archive_sessions` | `update_session` yalnız **bu** oturumu düzenler (title/working_dir/goal/tags/**archive**). `archive_sessions` **başka** oturumları toplu arşivler (soft, geri alınabilir): `idle_days` (N günden eski) + `title_contains` filtreleri, `dry_run` önizleme, `exclude` listesi; **daima `r.db`'ye bağlı → fiziksel olarak bu workspace'e scope'lu** ve **mevcut oturumu her zaman hariç tutar** (registry-build anında `SessionIDFrom(ctx)`). Bu, ajanın "eski oturumları temizle" isteğinde ham REST'e (`Invoke-RestMethod /api/sessions/{id}/state`) düşüp workspace scope'unu kaybetmesini önler — bir kez yanlış (default) workspace'i arşivleyen tam da o footgun'du. Kaynak: `builtin_sessionarchive.go`, `builtin_sessionupdate.go`, `builtin_sessions.go`. `SessionContextEnabled()` toggle'ıyla gated |
