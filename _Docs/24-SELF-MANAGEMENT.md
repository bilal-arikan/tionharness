...
| **Schedules** | `create_schedule` / `update_schedule` / `delete_schedule` / `list_schedules` / `run_schedule` | her yazım `reloadSchedules` ile scheduler'ı tazeler; `run_schedule` bir zamanlamayı **şimdi** elle tetikler (`runScheduleNow`) |
