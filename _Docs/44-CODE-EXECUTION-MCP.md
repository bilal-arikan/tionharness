...
graph LR
    ...
    API -->|localhost köprü| BR[TionHarness MCP proxy]
...
**Artı:**
- Anahtarsız değil, tam TionHarness kontrolünde — çağrı `mcp.pool`'dan geçtiği için
...
| Binding üreticisi | `internal/codemode/bindings.go` | `.tionharness/mcp/` altına `_bridge.py` + server-başına Python modülü; docstring = açıklama + normalize schema; her çağrıda sıfırdan regen |
...
| Gate | `internal/agent/tunables.go` + `codemode_tunable.go` + `internal/app/app.go` | `TIONHARNESS_CODE_MODE=1` (default KAPALI) **VE** `ShellEnabled` **VE** MCP kataloğu dolu |
...
**Akış:** model `run_code` (boş) → listing → `Read .tionharness/mcp/<module>.py` (tanım
...
- **Settings toggle:** `enableCodeMode` alanı (settings.json, Ayarlar → Geçişli
  ...
  `SetCodeMode`). `TIONHARNESS_CODE_MODE=1` artık doğrudan tunable değil, `EnableShell`
...
| 2. Tier-lazy (TionHarness bugünü) | 4 per-server özet satırı ≈ **69 tok**/tur (+~236 tok/aktive araç) | **−99,6%** |
...
- Araç olayları: 336 toplam; MCP-namespaced 130'un **yalnız ~5'i gerçek harici MCP**
  (kalanı CLI köprüsünün `tionharness_interaction`/`extended` built-in'leri) — harici MCP
...
- **Binding üretimi** (`codemode.WriteBindings(dir, entries, builtins, allow)`): MCP
  ...
  her fonksiyon `_bridge.call("tionharness__<tool>", args)` çağırır. MCP entry'leri
  namespaced isimle, built-in'ler bare isimle `allow`'dan geçer. Rezerve `tionharness`
  isimli gerçek bir MCP sunucusu çakışırsa `tionharness_server`'a yeniden adlandırılır.
