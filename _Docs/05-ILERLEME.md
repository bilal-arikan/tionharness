...
- **Neden:** Evolver ve fitness kodu bir hedeften yalnız `Primary`, `Guardrails`, `Scope`,
  `Status`, `Policy.Mode/MinRuns/CooldownHours` ve açıklama metnini okuyordu. Geri kalan
  alanlar (`Kind`, `Priority`, `Rubric`, `Summary`, `Notes`, `Questions`,
  `AutoApplySurfaces`, `auto` modu, `judge.rubricScore`) yalnız yazılıp gösteriliyordu;
  hiçbir kod uygulamıyor ya da ölçmüyordu. Ayrıca hedef **yalnız** `goal-writer` LLM
  çağrısıyla girilebiliyordu; metriği bilen kullanıcı için gereksiz kapı.
- **Model** (`internal/db/models_goal.go`): `Goal{Name, Description, RawText?, Status, Scope,
  Primary, Guardrails, Policy{mode propose|off, cooldownHours, minRuns}, CreatedBy,
  History}`. Eski JSON dosyalarındaki fazla alanlar yüklemede sessizce yok sayılır;
  `auto` modlu bir kayıt varsa `Validate` düzenlemede reddeder (kullanıcı `propose`'a alır).
- **Giriş yolları:** `POST /api/goals` (editörden doğrudan; `RawText`/provenance/geçmiş
  sunucunun) **eklendi**. `POST /api/goals/intake` (yazıcı ajan) kalır, artık zorunlu
  kapı değil "sözlerinle yazdır" yardımcısı. Yazıcı yanıtı doğrudan `db.Goal`'a
  parse edilir (`goals.DraftSchema` düzenlenebilir alt küme); sınırsız/katalog-dışı
  guardrail açık soru olmak yerine düşürülür, kullanıcı editörde ekler. `CanActivate`
  kalktı: etkinleştirme `Validate` ile aynı.
- **Katalog:** `judge.rubricScore` ve oto-uygulama yüzeyi allowlist'i kaldırıldı
  (yargıç ajan / oto-uygulama geldiğinde ilgili fazda geri eklenir, `_Docs/83` §4.5).
- **Ekran:** "Yeni hedef" boş formu açar (`GoalEditor` + `emptyGoal`), "Sözlerinle
  yazdır" yazıcı modalını. Editör/detaydan tür, öncelik, rubrik, oto-yüzey, açık
  soru ve not bölümleri gitti. Başlangıç hedefleri (`goals/defaults.go`) yeni modele
  uyarlandı (özet + not → açıklama).
- **Testler:** `goals/validate_test.go` (FromDraft `db.Goal` taslağı, `off` modu geçerli,
  `auto` bilinmeyen), `api/goals_test.go` (`handleCreateGoal` 201 / 422, provenance
...
TSK759. `stopSpawnQueue` yalnız dağıtıcıyı durduruyordu: daha önce başlatılmış
fire-and-forget turlar (spawn, worker, inbox teslimi) workspace yöneticisi DB'yi
kapatırken yazmaya devam ediyor, sonuç "database is closed" hataları oluyordu.

- **Bariyer:** `internal/agent/runtime.go` — `startBackgroundTurn` her arka plan
  turunu `spawnWG` altında başlatır (`Add(1)` **başlatan** goroutine'de, `go`'dan
  önce; böylece başlamak üzere olan tur kaçırılamaz). `closeBackgroundTurns`
  kapıyı kapatır, `cancelAllSessions` ile çalışanları iptal eder ve sınırlı süre
  (`backgroundTurnDrainGrace` = 15 sn, `internal/api`'deki `sessionTeardownGrace`
  ile aynı — depcheck yüzünden bilinçli kopya) bekler; süre dolarsa sessiz
  kalmayıp `logger.Error` basar.
- **Sıra sözleşmesi:** `CloseMCP` önce arka plan turlarını, sonra spawn kuyruğunu,
  en son koordinatör drain'lerini kapatır — kuyruk düşme bildirimleri yeni drain
  tetikleyebildiği için ters sıra birini arkada bırakırdı.
- **Kaçak yol:** `SpawnSession` boş slot bulduğunda kuyruğa uğramadan
  `launchSpawn`'a gidiyordu; oraya da `backgroundTurnsClosing()` guard'ı kondu.
  Bariyer kapalıyken hiçbir şey başlatılmaz ve çağıran kendi kaydını (slot,
  session kaydı, worker ctl) geri alır.
- **Test:** `internal/agent/runtime_close_test.go`. Testin üretim gecikmesini
  beklememesi için `spawnDrainGrace` test dikişi eklendi.

## Kuyruktaki mesaj çalışan tura canlı yönlendirme olarak taşınabiliyor (2026-09-06) ✅

TSK901. Kuyrukta bekleyen bir mesajı ancak tur bitince teslim edebiliyorduk: kullanıcı
"bunu şimdi söylemeliydim" dediğinde tek çaresi mesajı silip yeniden steer olarak
yazmaktı. Bunu istemci tarafında iki çağrıyla ("steer" + "kuyruktan sil") yapmak da
mümkün değildi — çağrılar arasında seri worker mesajı dispatch edebilir ve metin hem
yönlendirme hem kendi turu olarak koşar; ters sırada ise steer reddedildiğinde mesaj
kuyruktan silinmiş olur. İkisi de istemci tarafında telafi edilemez.

Yeni uç `POST /api/sessions/{id}/queue/{msgId}/steer` dönüşümü tek parçada yapıyor.
Atomikliği tek `withInbox` kilidi sağlıyor: arama, run'a teslim ve kuyruktan silme aynı
geri çağrının içinde; seri worker kuyruk başını aynı kilit altında popladığı için biz
kilidi tutarken mesajı dispatch edemiyor. Silme yalnız run metni **kabul ettikten
sonra** yapılıyor, dolayısıyla her red yolu (uçuşta tur yok → `404`, tur steerable değil
→ `200 {"result":"unsupported"}`, steer buffer dolu → `503`, mesajda ek dosya var →
`400`) kuyruğu olduğu gibi bırakıyor ve sebebini söylüyor: hiçbir rehberlik sessizce
tüketilmiyor.

- **`internal/api/steer_delivery.go`** (yeni): ortak `steerTargetRun` (yalnız uçuştaki
  turun sahibi run hedeflenir) + `deliverSteer` (native kanal vs. CLI stash ayrımı).
- **`internal/api/inbox_steer.go`** (yeni): `steerQueuedMessage` atomik dönüşümü ve
  `handleSteerQueued` uç noktası, her sonuç için ayrı `queuedSteerOutcome`.
- **`internal/api/inbox.go`**: `handleSessionControl` steer gövdesi paylaşılan
  yardımcılara indirgendi, davranış birebir korundu.
- **`internal/api/server.go`**: rota kaydı.
- **`frontend/src/api/chat.ts` + `useChatStream.ts`**: `steerQueued` istemcisi;
  **optimistik silme yok** — çip ancak sunucunun kuyruk güncellemesi mesajın gerçekten
  düştüğünü söyleyince kayboluyor, yani bir red mesajı UI'dan yok edemiyor.
- **`frontend/src/features/chat/PendingTray.tsx` + `ChatView.tsx`**: kuyruk çipinde
  "şimdi yönlendir" düğmesi, `activeStreaming` ile geçitli, devre dışıyken sebebi
  başlıkta.
- Testler: `internal/api/inbox_steer_test.go` (6), `PendingTray.test.tsx` (3).

Bilinen sınır: düğme turun gerçek steerability'siyle değil `activeStreaming` ile
geçitli, çünkü backend `chatRun.steerable` alanını hiçbir uçta/hub olayında yayınlamıyor
(TSK906 açık). `auto` modda düğme etkin görünüyor ve red ancak tıklamadan sonra
bildiriliyor.

Commit: `0168e5af` (`task/tsk901`).

## TOML sınırlayıcı doc yorumları gofmt'i susturdu (2026-09-06) ✅

TSK903. `internal/ingest/toml.go` haftalardır `gofmt -l` çıktısında duruyordu ve her
doğrulama turunda "bilinen sapma" diye elden geçirilmesi gerekiyordu — bu, gerçek bir
formatlama regresyonunu maskeleyecek bir gürültüydü. Sapma boşluk kaynaklı değildi:
Go 1.19+ doc yorumlarını eski godoc tipografi kuralıyla yeniden akıtıyor ve yorum
içindeki `''` çiftini tek bir tipografik sağ tırnağa (U+201D) çeviriyor. Yorumlar tam
da TOML'un üç-tek-tırnak sınırlayıcısını **birebir** anlattığı için gofmt'e uymak
dokümantasyonu bozmak anlamına geliyordu; bu yüzden dosya bilerek biçimlendirilmemişti.

Çözüm sınırlayıcıları girintili bir doc **kod bloğuna** taşımak: kod blokları
tipografik yeniden yazımdan muaf, dolayısıyla `"""` ve `'''` birebir hayatta kalıyor ve
`gofmt -l internal/` nihayet boş dönüyor. Kod yolu değişmedi; yalnız iki doc yorumu.

- **`internal/ingest/toml.go`**: `parseSimpleTOML` ve `tripleQuote` doc yorumlarında
  sınırlayıcılar düz cümle içinden girintili kod bloğuna alındı (14+/5-).

Bilinen sınır: godoc çıktısında sınırlayıcılar artık cümlenin içinde değil ayrı bir
kod bloğunda görünüyor — anlam aynı, akış biraz daha parçalı. Kartın diğer yarısı
(`internal/api/inbox.go:697` kopuk yorum cümlesi) TSK900 ile birlikte temizlenmişti.

Commit: `ca6bc837` (`task/tsk903`).

## claude-cli `read-only` modda steer artık "steered" diye yalan söylemiyor (2026-09-06) ✅

TSK900. `steerableForTurn` claude-cli + `read-only` için `true` dönüyordu: backend
kullanıcıya `"steered"` cevabı veriyor, ama mesaj pratikte hiçbir zaman enjekte
edilmiyordu — yalnız tur bitiminde `steer_undelivered` yoluyla kuyruğa düşüyordu.
Kullanıcı açısından bu, tutulmayan bir sözdü: "yönlendirildi" görüp davranışta
hiçbir değişiklik görmüyordu.

Kartın öncülü kısmen yanlıştı. Permission-prompt aracı `read-only` modda da
**bağlı** (`internal/climcp/climcp.go:97-102`), yani "araç bağlanmıyor" açıklaması
tutmuyor. Gerçek sebep, `read-only`'nin ayrıca `--permission-mode plan` koşması:
mutasyonları CLI kendi bloklar, köprülenmiş MCP araçları `--allowedTools`'tan
geçer, dolayısıyla prompt'a ulaşan tek çağrı `ExitPlanMode` kalır — ve
`callPermission` onu `steerContext()`'e hiç uğramadan `callExitPlan`'a sapıtır
(`internal/api/mcp_interaction_tools.go:99-101`). Enjeksiyon sınırı yok.

- **`internal/api/chat_control.go`**: `steerableForTurn` claude-cli için
  `return mode == "ask"` — `read-only` ve `auto` artık dürüstçe desteklenmiyor
  sayılıyor, `inbox.go` `"unsupported"` döndürüp mesajı kuyruğa alıyor.
- **`internal/api/inbox.go`**: `:697`'deki ön-mevcut kopuk yorum cümlesi
  temizlendi (TSK903'ün bir yarısı).
- **Testler**: `internal/api/steer_cli_test.go` — `TestSteerableForTurn`'ün
  read-only vakası `false`'a çevrildi, yeni `TestSteerableForTurnReadOnlyCLI` hem
  `read-only=false` hem `ask=true` yönünü pinliyor (tek yönlü değil, iki yönlü);
  `internal/api/autonomous_interaction_test.go` uyarlandı.

Bilinen sınır: plan onaylanıp CLI plan modundan çıkarsa steer teknik olarak teslim
edilebilir hale gelir, ama `steerable` tur kurulumunda bir kez hesaplanıyor
(`chat_turn_phases.go:196`) ve o anda bu bilinemez — yaygın durumda dürüst cevap
`false`. Ayrıca `TestAutonomousInteractionRecordsSteerable` beklenen değeri
fonksiyonun kendisinden türetiyor, değeri pinlemiyor (TSK905).

Dokümanlar: `_Docs/59-CLI-STEER-PLANI.md`.

Commit: `c98bba38` (`task/tsk900`).
...
Kart: **TSK747**. Async alt-ajan modu kaldırıldığında `wait` alanı, donmuş bir
prompt epoch'undan gelebilecek `wait:"sync"` çağrıları kırılmasın diye şemada
"deprecated no-op" olarak bırakılmıştı. Bu kalıntı artık silindi:
`runSubagentInput.Wait` alanı, `InputSchema`'daki `"wait"` property'si ve
`Call` içindeki `wait:"async"` reddi (`internal/tools/subagent.go`) kaldırıldı;
`internal/tools/subagent_test.go`'daki `TestRunSubagentWaitSyncIsIgnored` testi
ile enum tablosundaki iki `wait` vakası düştü (`context` ekseninin enum ve
normalizasyon testleri olduğu gibi duruyor).

Kaldırmanın güvenli olduğunun kanıtı — kartın beklediği "donmuş epoch kalmasın"
ön koşulu iki bağımsız yoldan sağlanıyor:

- **Şema doğrulaması sert değil.** Araç girdisi `parseInput` üzerinden düz
  `json.Unmarshal` ile çözülüyor (`internal/tools/toolbuilder.go:25`);
  `DisallowUnknownFields` yok. Şemadaki `additionalProperties:false` yalnız
  sağlayıcıya gönderilen sözleşmedir ve o da **aynı turda gönderilen** şemaya
  göre doğrulama yapar. Dolayısıyla eski bir epoch'tan gelen fazladan `wait`
  anahtarı hata değil, sessiz yok sayma üretir.
- **Diskte donmuş araç şeması yok.** 1035 canlı `prompt_epoch.json` yan
  dosyasının tamamı tarandı: hiçbirinde kalıcılaşmış araç dizisi (dolayısıyla
  `wait` taşıyan bir `run_subagent` şeması) yok — dosyalarda geçen 670
  `run_subagent` ifadesi donmuş **system prompt** metnindendir. Yeniden
  başlatmadan sonra `EpochToolDefs` şemayı canlı kayıttan yeniden üretir
  (`internal/agent/promptepoch.go:303`), ayrıca bellekteki epoch bir saatlik
  boşluktan sonra zaten `ttl-cold` ile tazelenir.

Dokümanlar: `_Docs/25-SUBAGENT-ISOLATION.md` (eksen bölümü + "Senkron-tek mod"),
`_Docs/24-SELF-MANAGEMENT.md` (araç tablosu).

## Steer (canlı yönlendirme) taşıyıcı matrisi araştırması (2026-09-06) ✅

TSK762 — **araştırma kartı**; üretim kodu değişmedi, yalnız doküman güncellendi.
Soru: "Yönlendir" neden çalışmıyor görünüyor, hangi taşıyıcıda gerçekten mümkün?

Bulgular: (1) claude-cli'da steer teslimi tamamen permission-prompt aracına bağlı,
o araç ise **varsayılan `auto` izin modunda hiç bağlanmıyor** — yani en yaygın
kurulumda steer yapısal olarak imkânsız, backend dürüstçe `"unsupported"` dönüp
mesajı kuyruğa alıyor; `read-only` ise `steerableForTurn`'de `true` görünmesine
rağmen pratikte teslim etmiyor. (2) Native (doğrudan API) yolda `drainSteer`
yalnız araç döngüsü iterasyon başlarında çağrıldığı için **araçsız turda mesaj
sessizce kayboluyor** — ne teslim ediliyor ne kuyruğa düşüyor. (3) codex-cli
`exec --json` + tek atımlık stdin mimarisi nedeniyle hiçbir modda mümkün değil;
claude-cli'ın kalıcı stdin'ine ikinci bir kullanıcı mesajı yazmak protokolce
kabul edilse de CLI onu tur sonuna ertelediği için kazanç getirmiyor.

Doküman: `_Docs/59-CLI-STEER-PLANI.md` — yeni "Doğrulanmış durum" bölümü
(taşıyıcı × izin modu matrisi, dosya:satır kanıtlarıyla) plan metninden ayrıldı.

Açılan takip kartları: **TSK899** (araçsız turda steer kaybı + buffer dolu iken
sessiz düşürme), **TSK900** (`read-only` modda yanlış "steerable" raporu),
**TSK901** (kuyruktaki mesajı steer'e çeviren uç + UI).
...
## Hedefler: yeni workspace üç başlangıç hedefiyle geliyor (2026-09-13) ✅

- **İstek:** varsayılan workspace'in neler getirdiğine bakıp olası hedefler belirlemek.
- **Durum tespiti:** yeni workspace bir pano, bir başlangıç akışı (`starter-answer-verify`)
  ve üç **kapalı** otomasyon (`board-run-in-progress`, `board-archive-done`,
  `insight-apply-workspace-opt`) getiriyordu; **hiç hedef getirmiyordu** — Hedefler
  ekranı kullanıcı kendi cümlesiyle bir hedef yazana kadar boştu.
- **Backend:** `goals.EnsureDefaultGoals` (yeni `internal/goals/defaults.go`) üç
  başlangıç hedefini **taslak** olarak tohumlar: `usage.costUSDPerSession` (min),
  `usage.cacheHitRatio` (max), `session.humanAsksPerSession` (min). Üçü de
  guardrail'li ve birbirini dengeler: 1. hedefin guardrail'i 3. hedefin primary
  metriğidir, böylece "ucuzluk" kullanıcıya daha çok soru sorarak, "az soru" da
  yanlış varsayımla ilerleyerek kazanılamaz.
- **Metrik seçimi:** yalnız katalogda `Available` olan metrikler kullanıldı; reçete
  metrikleri (düz sohbet workspace'inde reçete koşmaz), `board.cycleTimeSec` (ilgili
  otomasyon kapalı geldiği için ölçmez) ve `Available:false` olanlar
  (`judge.rubricScore`, `feedback.upRatio` …) bilerek dışarıda bırakıldı — ölçülmeyen
  metrik üzerine hedef sonsuza dek "henüz ölçülmedi" der.
- **Tohumlama sözleşmesi:** akış/otomasyon tohumlarıyla aynı — `.seeded-goals.json`
  defteri kullanıcı silmesini kalıcı yapar (silinen hedef geri gelmez), idempotent,
  eski workspace'ler bir sonraki açılışta backfill olur. `open()` içinde
  `EnsureDefaultAutomations`'tan sonra çağrılır.
- **Provenance:** `db.GoalBySeed = "seed"` eklendi; `authorLabel` bunu "Başlangıç
  hedefi" olarak gösteriyor. Hedefler **taslak** geldiği için kullanıcı gözden
  geçirip etkinleştirene kadar hiçbir ölçüm ya da evrim önerisi çalışmaz.
- **Test:** `internal/goals/defaults_test.go` — her tohum `Validate`+`CanActivate`
