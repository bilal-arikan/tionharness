# TionSwarm — Native Masaüstü Penceresi (Seçenek 2)

> Durum: **UYGULANDI ✅ (2026-06-22)**. `tionswarm-desktop.exe` çift tıkla → tarayıcı
> sekmesi yerine **kendi WebView2 penceresinde** açılır. Başsız `tionswarm.exe` aynen durur.
>
> Gerçekleşen mimari aşağıdaki planla birebir: ayrı `internal/app` boot paketi (paylaşılan),
> `cmd/tionswarm-desktop` (Windows-only, `jchv/go-webview2`, CGO'suz), `:0` boş port,
...
**Felsefe kısıtı:** Mevcut `cmd/tionswarm` (başsız sunucu) **saf-Go, CGO'suz, çapraz-derlenebilir**
...
```
cmd/
├── tionswarm/            # mevcut başsız sunucu (DEĞİŞMEZ, saf-Go, çapraz-derleme)
│   └── main.go
└── tionswarm-desktop/    # YENİ: native pencere sarmalayıcı (Windows-only, build-tag)
    └── main.go
```
...
`cmd/tionswarm/main.go` şu an tüm boot'u inline yapıyor (config → secret → settings →
...
- `cmd/tionswarm/main.go` ~100 satırdan ~15 satıra iner: `app.Bootstrap` + signal + `Serve`.
...
cmd/tionswarm-desktop/main.go      # //go:build windows  — webview sarmalayıcı
cmd/tionswarm-desktop/stub_other.go # //go:build !windows — boş stub + "yalnız Windows" notu
cmd/tionswarm-desktop/window_windows.go, openwindow_windows.go, titlebar_windows.go, dpi_windows.go  # Windows-özel pencere/DPI/titlebar
```

### `cmd/tionswarm-desktop/main.go` (taslak)
...
- Mevcut `cmd/tionswarm` build'i **etkilenmez** (yeni paket yalnız desktop binary'sinde import edilir).
...
```powershell
.\scripts\build.ps1            # tionswarm.exe (başsız sunucu, mevcut)
.\scripts\build.ps1 -Desktop   # tionswarm-desktop.exe (native pencere, UI gömülü)
```

- `-Desktop`: UI build (aynı) + `go build -ldflags "-H windowsgui -s -w" -o tionswarm-desktop.exe ./cmd/tionswarm-desktop`
...
İlk sürümde atlanabilir; ikon istenirse `go-winres` ile `cmd/tionswarm-desktop/winres/`
...
| Başsız sunucu hâlâ isteniyor | `tionswarm.exe` aynen durur; iki dağıtım yan yana |

## Başlık çubuğu tema uyumu ✅ (2026-06-23)

Native başlık çubuğu (caption + küçült/büyüt/kapat butonları + kenarlık) uygulama temasına
boyanır. **`cmd/tionswarm-desktop/titlebar_windows.go`** (`//go:build windows`, salt `syscall`,
`dwmapi.dll`):
...
Not: başsız `tionswarm.exe` zaten konsollu olduğundan etkilenmezdi; bayrak orada da zararsız.
...
olarak büyütür → bulanıklık. **Çözüm:** `cmd/tionswarm-desktop/dpi_windows.go` (salt `syscall`,
...
- macOS/Linux için `webview/webview_go` (CGO) ile `cmd/tionswarm-desktop/main_unix.go`
...
1. **`-H windowsgui` + log**: konsol olmayınca stdout logları görünmez. Çözüm: logbuf
   zaten UI "Loglar" ekranında; ayrıca dosyaya log opsiyonu (`TIONSWARM_LOG_FILE`) eklenebilir.
...
4. **İki binary**: dağıtımda iki exe (`tionswarm.exe` server, `tionswarm-desktop.exe` masaüstü).
   İstenirse ileride tek exe + `--server` flag'iyle birleştirilebilir (ayrı iş).

## Doğrulama planı

1. `go build ./...` + `go vet` (tüm hedefler) yeşil; başsız `tionswarm.exe` davranışı değişmedi
   (smoke: `/health`, `/` HTML 200 — mevcut testler).
2. `tionswarm-desktop.exe` çift tıkla → pencere açılır, UI yüklenir, sohbet/akış çalışır.
...
1. `internal/app` paketi: boot dizisini taşı (davranış-korumalı refactor) + `cmd/tionswarm` onu kullansın.
2. `go get github.com/jchv/go-webview2`.
3. `cmd/tionswarm-desktop/main.go` (`//go:build windows`) + `!windows` stub.
