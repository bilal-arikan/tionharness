...
>   StatusNotifierItem). TionHarness'nun "tek binary, çapraz-derleme, minimal bağımlılık"
...
> TionHarness'ya değer katacak. **Plan — uygulanmadı; ileride eklenebilecek featureler.**

- [ ] **HA-1 — Gelişmiş hafıza: tam-metin arama + LLM özet + kullanıcı modelleme** *(yüksek değer)*:
  the external agent hafızası üç katman taşıyor — (a) **FTS5 tam-metin arama** oturumlar üzerinde (TionHarness'da
  mevcut **CG-16** ile örtüşür; Go tarafında ripgrep veya bleve/saf-Go ters-indeks ile, DB-siz
  felsefeye uygun), (b) **LLM-destekli özetleme** ile çapraz-oturum recall (TionHarness'da `Reflect`
  ...
  (tercihler/bağlam/davranış). TionHarness'nun mevcut lexical-cosine recall'ı (Faz 6) bunun altyapısı;
...
- [ ] **HA-2 — Kendini-geliştiren prosedürel skill + skill hub** *(yüksek değer — ayırt edici)*:
  ...
  [agentskills.io](https://agentskills.io) merkezi hub'ında paylaşılır. TionHarness'da skill sistemi
...
> **Not:** İkisi de TionHarness'nun mevcut alt sistemlerinin (Faz 6 hafıza, skill sistemi, market,
  ...
  **HA-2**, `21-MARKET.md`.
  ...
  - **Önkoşul iyileştirmeler (entegrasyondan ÖNCE — kendi skill sistemimizde).**
    ...
    - [x] **SK-1 ✅ (2026-06-23) — Çok-dosyalı skill (bundled resources):** skill bir KLASÖR olabilsin; SKILL.md gövdesinin
      ...
      `${CLAUDE_SKILL_DIR}`/`${CLAUDE_SESSION_ID}` ikamesi → gömülü dosyalara/scriptlere atıf. TionHarness'da
    ...
    - [x] **SK-2 ✅ (2026-06-23) — Ölçeklenebilir keşif (`skill_search` + koşullu `paths:`):**
      ...
      `.claude/skills` keşfi. TionHarness'da: `paths:` koşullu aktivasyon (PostToolUse/fs-touch ile) + `skill_search`
    ...
    - [x] **SK-3 ✅ (2026-06-23) — `allowed_tools` enforcement:** `AlwaysAllow` bugün yalnız UI'da, **uygulanmıyor**. **CC deseni:**
      ...
      TionHarness'da skill yüklenince ilan ettiği araçları oturum/skill kapsamında scope/auto-allow et.
    - [x] **SK-4 ✅ (2026-06-23) — Zengin frontmatter passthrough:** CC `version`/`disable-model-invocation`/`user-invocable`/
      ...
      (provenance) + `user-invocable` TionHarness'ya eklensin; gerisi degrade-gracefully korunur (parser zaten bilinmeyen
      anahtarı tutuyor).
...
> TionHarness'nun da yaşayacağı gerçek mimari boşluklara birebir oturuyor — çünkü ürün de
...
- [ ] **RG-3 — Kaynak kira (lease) registry** *(2. dalga, M, orta değer)*: ajan port/temp-dir ister, runtime boş olanı verir, `stop`'ta otomatik bırakır. `:8090` çakışması bir daha olmaz; TionHarness'nun kendi açılış preflight'ı için de kullanılır. Yer: `agent/runtime.go` yaşam döngüsü.
