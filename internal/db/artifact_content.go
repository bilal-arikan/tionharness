...
func (d *DB) ImportMediaSource(sessionID, src string) (string, error) {
	...
	if err != nil {
		...
		// filesystem than TionHarness — so the path is valid THERE but not accessible
		// HERE. Point the caller at the real fix instead of a bare "not found".
		if filepath.IsAbs(src) {
			return "", fmt.Errorf("source file not accessible from TionHarness: %s — an absolute path from an MCP server that runs in a container/remote host or a different filesystem does not resolve here; have that tool return the file CONTENT (e.g. base64) instead of a host path, or reference a file inside the workspace", src)
		}
		if wdTried != "" {
