---
name: "TionHarness Progress Notes"
description: "How to keep durable, cross-session progress for long-running work in TionHarness: the automatic todo_write progress file (<cwd>/.tionharness/progress.json) that the next session resumes from, plus the human-readable PROGRESS.md convention you maintain with the file tools. A progress.txt + feature_list.json style note-taking pattern for long-horizon agents."
...
access: shared
---
# TionHarness — Progress Notes (durable, cross-session)
...
session *reads the notes to get up to speed* instead of starting blind. TionHarness
...
- **Location:** `<cwd>/.tionharness/progress.json` (when the session has a working
