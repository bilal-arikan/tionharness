...
- **Döngü kasıtlıdır.** `evaluate → decide → generate` bir **geri-kenar (cycle)**'dır.
  TionHarness orchestration motoru döngülere **izin verir** ve `maxSteps` (50) ile sınırlar —
...
- Bu desen `[[tionharness-flows]]` üzerine kuruludur (flow node tipleri/şablon değişkenleri orada).
