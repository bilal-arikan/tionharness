...
color: "#14b8a6"
---
# TionHarness — Doc Improver (audit & tighten context docs)
...
— a repeatable discipline adapted to TionHarness's own doc surfaces.

> TionHarness's context lives in many places (skills, workspace rules, `_Docs`), so this
...
| **Reference context** | `_Docs/*.md`, the `tionharness-project`-style reference skill | *Reference* (knowledge) |
...
3. **Non-obvious patterns (gotchas)** — the traps a fresh agent would hit
   ...
   IPv6 fix). These are the highest-value lines in any TionHarness doc — never cut them.
...
## The #1 TionHarness anti-pattern: changelog-leak

Reference docs (especially `tionharness-project` and `_Docs` mechanics sections) tend
...
- **Reference docs / always-loaded context** (workspace CLAUDE.md, the
  `tionharness-project` reference skill): brevity matters *most* — these sit in the
  prompt every turn. Trim hard; delegate detail to `_Docs`.
- **Action skills loaded on demand** (the `tionharness-*` default skills): the body is
