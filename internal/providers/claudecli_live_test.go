...
// path: it spends actual tokens, so it is gated behind TIONHARNESS_LIVE_CLI=1 and
...
//
//	TIONHARNESS_LIVE_CLI=1 go test ./internal/providers/ -run TestLiveClaudeCLIResume -v
func TestLiveClaudeCLIResume(t *testing.T) {
	if os.Getenv("TIONHARNESS_LIVE_CLI") != "1" {
		t.Skip("set TIONHARNESS_LIVE_CLI=1 to run the live claude-cli resume test")
	}
	bin := "claude"
...
// behind TIONHARNESS_LIVE_CLI=1 (spends tokens).
//
//	TIONHARNESS_LIVE_CLI=1 go test ./internal/providers/ -run TestLivePersistentSession -v
func TestLivePersistentSession(t *testing.T) {
	if os.Getenv("TIONHARNESS_LIVE_CLI") != "1" {
		t.Skip("set TIONHARNESS_LIVE_CLI=1 to run the live persistent-session test")
	}
	bin := "claude"
	if p := os.Getenv("TIONHARNESS_CLAUDE_BIN"); p != "" {
		bin = p
