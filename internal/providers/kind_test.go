...
func TestCatalogDerivedFromKinds(t *testing.T) {
	cat := Catalog()
	if len(cat) != 8 {
		t.Fatalf("catalog: want 8 entries, got %d", len(cat))
	}
	wantOrder := []string{"claude-cli", "anthropic", "minimax", "minimax-anthropic", "openrouter", "zai", "deepseek", "deepseek-anthropic"}
