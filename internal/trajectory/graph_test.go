...
func TestTrajSeedFromRecipe(t *testing.T) {
	...
	tr := Seed(root, testRecipeSpec(), "plan-dev@3")
	...
	if n := NodePtr(&tr, "a:summarize-board@code"); n == nil || n.PhaseID != "p:code" || n.State != db.TrajStateGhost {
		t.Fatalf("phase watcher = %+v", n)
	}
	if n := NodePtr(&tr, "a:update-docs"); n == nil || n.PhaseID != "" || n.Kind != db.TrajNodeAutomation {
		t.Fatalf("global watcher = %+v", n)
	}
	if n := NodePtr(&tr, OptimizerNodeID); n == nil || n.RefID != "recipe-optimizer" {
		t.Fatalf("optimizer = %+v", n)
	}
	if bare := Seed(root, nil, ""); len(bare.Nodes) != 1 || bare.TemplateRef != "" {
...
func TestTrajPhaseTransitionsAndStatus(t *testing.T) {
	tr := Seed(db.Session{ID: "SES1"}, testRecipeSpec(), "r@3")
	DeriveStatus(&tr)
	if tr.Status != db.TrajStatusPlanned {
		t.Fatalf("status before start = %s", tr.Status)
	}
	AutoStartPhase(&tr, 10)
	if ActivePhase(&tr) != "p:plan" || NodePtr(&tr, "p:plan").StartMs != 10 {
		t.Fatalf("auto start = %+v", Phases(&tr))
	}
	AutoStartPhase(&tr, 20) // no-op while one is active
	if ActivePhase(&tr) != "p:plan" {
	...
	if err := SetPhaseState(&tr, "code", db.TrajStateActive, "", 30); err != nil {
		t.Fatal(err)
	}
	if p := NodePtr(&tr, "p:plan"); p.State != db.TrajStateDone || p.EndMs != 30 {
		t.Fatalf("previous phase after activation = %+v", p)
	}
	if err := SetPhaseState(&tr, "nope", db.TrajStateActive, "", 30); err == nil {
		t.Fatal("unknown phase must be refused")
	}
	if err := SetPhaseState(&tr, "code", "bogus", "", 30); err == nil {
		t.Fatal("unknown state must be refused")
	}
	// A gate opens: waiting beats running.
	AddNode(&tr, db.TrajectoryNode{ID: "g:ASK1", Kind: db.TrajNodeGate, Origin: db.TrajOriginObserved, State: db.TrajStateActive})
	DeriveStatus(&tr)
	if tr.Status != db.TrajStatusWaiting {
		t.Fatalf("status with open gate = %s", tr.Status)
	}
	NodePtr(&tr, "g:ASK1").State = db.TrajStateDone
	if err := SetPhaseState(&tr, "code", db.TrajStateDone, "", 40); err != nil {
		t.Fatal(err)
	}
	DeriveStatus(&tr)
	...
	tr2 := Seed(db.Session{ID: "SES2"}, testRecipeSpec(), "r@3")
	_ = SetPhaseState(&tr2, "plan", db.TrajStateFailed, "planner gave up", 5)
	DeriveStatus(&tr2)
	if tr2.Status != db.TrajStatusFailed || NodePtr(&tr2, "p:plan").Reason != "planner gave up" {
		t.Fatalf("failed phase → %s / %+v", tr2.Status, NodePtr(&tr2, "p:plan"))
	}
...
func TestTrajApplyPlan(t *testing.T) {
	tr := Seed(db.Session{ID: "SES1"}, nil, "")
	if err := ApplyPlan(&tr, nil); err == nil {
		t.Fatal("empty plan must be refused")
	}
	if err := ApplyPlan(&tr, []PlanPhase{{ID: "Bad Id"}}); err == nil {
		t.Fatal("invalid id must be refused")
	}
	if err := ApplyPlan(&tr, []PlanPhase{{ID: "a"}, {ID: "a"}}); err == nil {
		t.Fatal("duplicate id must be refused")
	}
	if err := ApplyPlan(&tr, []PlanPhase{{ID: "a", GateKind: "magic"}}); err == nil {
	...
	if err := ApplyPlan(&tr, plan); err != nil {
		t.Fatal(err)
	}
	phases := Phases(&tr)
	...
	_ = SetPhaseState(&tr, "research", db.TrajStateActive, "", 1)
	// Hang a worker under the active phase; a re-plan that drops that phase is refused.
	AddNode(&tr, db.TrajectoryNode{ID: "s:W1", Kind: db.TrajNodeSession, Origin: db.TrajOriginObserved, PhaseID: "p:research", Lane: 1, State: db.TrajStateActive})
	if err := ApplyPlan(&tr, []PlanPhase{{ID: "build"}}); err == nil || !strings.Contains(err.Error(), "research") {
	...
	if err := ApplyPlan(&tr, []PlanPhase{{ID: "research"}, {ID: "build"}, {ID: "ship"}}); err != nil {
		t.Fatal(err)
	}
	if NodePtr(&tr, "p:docs") != nil || NodePtr(&tr, "p:ship") == nil {
		t.Fatalf("re-plan nodes = %+v", Phases(&tr))
	}
	if ActivePhase(&tr) != "p:research" || NodePtr(&tr, "s:W1").PhaseID != "p:research" {
...
func TestTrajRender(t *testing.T) {
	tr := Seed(db.Session{ID: "SES1", Title: "Root"}, testRecipeSpec(), "plan-dev@3")
	tr.ID, tr.Revision, tr.Status = "RTA7", 4, db.TrajStatusRunning
	AutoStartPhase(&tr, 1)
	AddNode(&tr, db.TrajectoryNode{ID: "s:W1", Kind: db.TrajNodeSession, Origin: db.TrajOriginObserved, Label: "Planner", RefID: "W1", PhaseID: "p:plan", Lane: 1, State: db.TrajStateDone})
	AddNode(&tr, db.TrajectoryNode{ID: "g:ASK1", Kind: db.TrajNodeGate, Origin: db.TrajOriginObserved, Label: "ask", RefID: "ASK1", Lane: 0, State: db.TrajStateActive})
	out := Render(&tr)
	...
	bare := Seed(db.Session{ID: "SES2"}, nil, "")
	if out := Render(&bare); !strings.Contains(out, "none declared") {
