...
// handleListBuiltinHooks returns the read-only list of TionHarness's auto-injected tool
...
func (s *Server) handleListBuiltinHooks(w http.ResponseWriter, _ *http.Request) {
	...
	list := []builtinHook{
		...
		},
		{
			...
			Event:       "System",
			Enabled:     true,
			Description: "In ask / read-only modes the CLI's ExitPlanMode is routed through TionHarness's plan-approval card; in auto mode plan tools are disallowed so the agent just executes.",
		},
		{
