...
// the gap between TionHarness's own segment estimate (TotalTokens) and the real prompt
// the underlying CLI actually sends to the model. The CLI injects its OWN system
// prompt + tool schemas + MCP bridge that TionHarness never composes or sees, so for
...
func (s *Server) computeCachePreview(provider string, session db.Session, msgCount int, hasSystem, hasDynamic, hasTools bool) cachePreview {
	...
	case "anthropic":
		...
		return c
	case "openrouter":
		// TionHarness sends an Anthropic-style cache_control breakpoint on the static
	...
	default:
		return cachePreview{Mode: "none", Note: "Bu sağlayıcı için TionHarness cache breakpoint göndermez → istek her tur taze."}
	}
}
...
// averaged per call) and compared against TionHarness's own segment estimate, so the
