...
func TestInteractionTokenStablePerSessionAgent(t *testing.T) {
	...
	t1 := runs.interactionToken("ws1", "s1", "a1")
	t1b := runs.interactionToken("ws1", "s1", "a1")
	if t1 == "" || t1 != t1b {
	...
	if t2 := runs.interactionToken("ws1", "s1", "a2"); t2 == t1 {
	...
	run := runs.register("run-1", "s1", "ws1", func() {})
	...
	if t1c := runs.interactionToken("ws1", "s1", "a1"); t1c != t1 {
...
func TestByTokenPerRunFallback(t *testing.T) {
	runs := newChatRuns()
	run := runs.register("run-x", "s9", "ws1", func() {})
