...
func TestCatalogDisplayName(t *testing.T) {
	tests := []struct {
		...
		{"lazy built-in", true, "mcp__tionswarm_extended__activate_tools"},
		{"interaction MCP idempotent", true, "mcp__tionswarm_interaction__activate_tools"},
		{"extended MCP idempotent", true, "mcp__tionswarm_extended__notify"},
		{"external MCP", true, "mcp__codebase-memory-mcp__search_code"},
		{"native", false, "Read"},
	}
	inputs := []string{
		"activate_tools",
		"mcp__tionswarm_interaction__activate_tools",
		"mcp__tionswarm_extended__notify",
