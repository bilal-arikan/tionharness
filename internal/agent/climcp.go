...
// cliNativeToolAllowlist returns the claude-cli BUILT-IN tools a bridged turn may
// keep, for the `--tools` flag. It is the positive mirror of the suppression list
// writeCLIMCPConfig assembles: the same conditions, inverted. The point is prompt
// size, not permission — `--disallowedTools` leaves the CLI's system prompt at
// its full ~36k tokens (the suppressed tools still shape it), while `--tools`
// with the five file tools + ToolSearch measures ~11k (2.1.259, _Docs/17). Every
// entry below is either always needed (file tools; ToolSearch, which reaches the
...
// session fingerprint.
func cliNativeToolAllowlist(ag db.Agent, inter tools.InteractionEndpoint, mode string, shellEnabled bool) []string {
	...
	if !shellEnabled {
		out = append(out, "Bash", "BashOutput", "KillShell", "TaskOutput", "TaskStop")
	}
	// WS17 invariant (see writeCLIMCPConfig): a native whose bridge is not advertised
	// this turn stays on the menu so the model is never left without the family.
	if !advertised["todo_write"] {
		out = append(out, "TodoWrite", "TaskCreate", "TaskUpdate", "TaskList", "TaskGet")
	}
	...
	if mode == "ask" || mode == "read-only" {
		out = append(out, "EnterPlanMode", "ExitPlanMode")
	}
	return out
}
