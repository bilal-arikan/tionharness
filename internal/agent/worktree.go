...
func gitRepoToplevel(ctx context.Context, dir string) string {
	cmd := proc.CommandContext(ctx, "git", "-C", dir, "rev-parse", "--show-toplevel")
	out, err := cmd.Output()
	if err != nil {
		return ""
	}
...
func (r *Runtime) ensureWorktree(ctx context.Context, base, sessionID string) string {
	...
	if out, err := addCmd.CombinedOutput(); err != nil {
		retryCmd := proc.CommandContext(ctx, "git", "-C", repo, "worktree", "add", wtPath)
...
func (r *Runtime) RemoveSessionWorktree(sessionID string) {
	...
	rmCmd := proc.Command("git", "-C", wtPath, "worktree", "remove", "--force", wtPath)
