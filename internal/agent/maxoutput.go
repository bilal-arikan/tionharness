...
// SWARMGO_MAX_OUTPUT_TOKENS; 0/unset defers to the per-family default. Mirrors the
// SWARMGO_MAX_TOOL_ITERS / SWARMGO_MAX_CONTEXT_TOKENS escape hatches.
var maxOutputOverride = envMaxOutputOverride()

func envMaxOutputOverride() int {
	if v, err := strconv.Atoi(os.Getenv("SWARMGO_MAX_OUTPUT_TOKENS")); err == nil && v > 0 {
		return v
	}
	return 0
}
...
// SWARMGO_MAX_OUTPUT_TOKENS env; then the per-model family default. When none
