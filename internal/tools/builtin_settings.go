...
func (UpdateSettingsTool) Def() providers.ToolDef {
	return providers.ToolDef{
		...
		Description: "Change one or more application-wide settings and ACTIVATE them immediately (persisted to settings.json AND pushed into the live runtime — no restart). Pass a `patch` object containing only the fields you want to change, using the exact field names from get_settings (e.g. {\"theme\":\"light\"}, {\"autoTitleEnabled\":false}, {\"enableShell\":true}, {\"defaultPermissionMode\":\"ask\"}). Provider/model is not an app-global setting — it is configured per agent. Numeric fields are clamped to safe ranges. Write-only secret fields are accepted: `anthropicKey` / `minimaxKey` (\"\" clears). Always read get_settings first so you patch the right keys. Returns the new masked settings.",
		...
		Examples: []json.RawMessage{
			...
			json.RawMessage(`{"patch":{"language":"en","autoTitleEnabled":false}}`),
