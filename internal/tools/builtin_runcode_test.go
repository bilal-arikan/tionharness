...
func TestRunCodeDiscoveryListsModules(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".tionharness", "mcp", "demo.py")); err != nil {
		t.Fatalf("bindings not written: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, ".tionharness", "mcp", "_bridge.py")); err != nil {
...
func TestRunCodeDispatchesBuiltinModule(t *testing.T) {
	...
	callBI := func(_ context.Context, name string, _ json.RawMessage) (mcp.CallToolResult, error) {
		gotName = name // must be the BARE built-in name, not the tionharness__ namespace
	...
	script := "from tionharness import echo_tool\nprint('ok:', echo_tool(x=1)['echoed'])"
	args, _ := json.Marshal(map[string]any{"script": script})
	out, err := tool.Call(t.Context(), args)
	if err != nil {
		t.Fatalf("run_code failed: %v", err)
	...
	if !strings.Contains(out, "tionharness__echo_tool") {
...
// servers: the discovery listing shows the tionharness built-ins module.
func TestRunCodeBuiltinsOnlyDiscovery(t *testing.T) {
	...
	if !strings.Contains(out, "tionharness: list_flows") {
		t.Fatalf("listing must name the tionharness module + function, got:\n%s", out)
	}
	if _, err := os.Stat(filepath.Join(dir, ".tionharness", "mcp", "tionharness.py")); err != nil {
		t.Fatalf("tionharness bindings not written: %v", err)
	}
}
