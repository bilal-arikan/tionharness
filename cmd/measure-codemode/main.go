...
// configured in a SwarmGo data dir, pulls their REAL tool catalogs, and reports
...
//  2. tier-lazy   — SwarmGo's current default (MCP tools lazy + name-only:
...
// Usage:
//
//	go run ./cmd/measure-codemode [-data ~/.swarmgo] [-timeout 30s]
package main
...
func main() {
	...
	if home, err := os.UserHomeDir(); err == nil {
		defaultData = filepath.Join(home, ".swarmgo")
	}
	dataDir := flag.String("data", defaultData, "SwarmGo data dir (workspaces are scanned for MCP server configs)")
