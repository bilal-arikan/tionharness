...
func report(cfgs []mcp.ServerConfig, timeout time.Duration) {
	...
	bindDir, err := os.MkdirTemp("", "swarmgo-measure-*")
	if err != nil {
	...
	fmt.Printf("  2. tier-lazy (SwarmGo today)      : %s ≈ %6d tokens/turn (+~%d tokens per activated tool)\n",
...
// tierLazyCost estimates the per-turn cost of SwarmGo's current default for MCP
...
func discoveryListing(modules map[string][]string) string {
	...
	sort.Strings(names)
	var b strings.Builder
	b.WriteString("MCP bindings regenerated under .swarmgo/mcp/ (on PYTHONPATH for run_code scripts).\nModules:\n")
	for _, m := range names {
