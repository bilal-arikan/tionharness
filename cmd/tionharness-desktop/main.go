...
// Command tionharness-desktop runs TionHarness as a native desktop application: it
// boots the same in-process server as cmd/tionharness on a free loopback port, then
...
// running server via TIONHARNESS_WEBVIEW_URL. The primary process owns the server
// and exposes a JS bridge (tionharnessOpenWindow) that spawns those instances.
...
const envWebviewURL = "TIONHARNESS_WEBVIEW_URL"
...
// binds the tionharnessOpenWindow JS function so the UI can spawn more windows.
func runPrimary(logs *logbuf.Buffer, logger *slog.Logger) {
	...
	// JS bridge: window.tionharnessOpenWindow(route) spawns a new native window
	// (a connect-only secondary process) scoped to that hash route.
	if err := w.Bind("tionharnessOpenWindow", func(route string) {
		spawnWindow(base, route)
	}); err != nil {
		logger.Warn("bind tionharnessOpenWindow failed", "error", err)
	}
...
func runSecondary(logger *slog.Logger, target string) {
	...
	if err != nil || !isLoopback(u) {
		logger.Error("refusing non-loopback TIONHARNESS_WEBVIEW_URL", "url", target)
		return
	}
...
// newWindow creates the standard TionHarness WebView2 window (nil if the runtime is
// unavailable).
...
// windows can only ever render the local TionHarness server.
