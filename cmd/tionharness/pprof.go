...
// always-on cost, so they ride the same TIONHARNESS_PPROF gate as the endpoints
...
// startPprof launches a loopback-only profiling server when TIONHARNESS_PPROF is
...
//
// Override the bind address with TIONHARNESS_PPROF_ADDR (default 127.0.0.1:6060).
...
func startPprof(logger *slog.Logger) {
	v := os.Getenv("TIONHARNESS_PPROF")
	if v != "1" && v != "true" && v != "TRUE" {
		return
	}

	addr := os.Getenv("TIONHARNESS_PPROF_ADDR")
	if addr == "" {
		addr = "127.0.0.1:6060"
	}
