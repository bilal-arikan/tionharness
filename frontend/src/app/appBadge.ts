...
// host bridge (window.tionharnessSetBadge) is also called when the desktop shell
...
export function setAppBadge(count: number) {
  ...
  const host = window as unknown as { tionharnessSetBadge?: (n: number) => void }
  try {
    host.tionharnessSetBadge?.(count)
  } catch {
