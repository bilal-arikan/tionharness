...
export const VISIBLE_RELATIONS_PER_SIDE = 15
...
export function buildFocusGraph({
  ...
  for (const handle of [...lineage, ...parents, ...children]) {
  ...
  const ancestorKeys = lineage
    .map((handle) => refToString(handle.ref))
    .filter((key, index, keys) => key !== focusKey && keys.indexOf(key) === index)
  const ancestorKeySet = new Set(ancestorKeys)
  const allLeft = [
    ...ancestorKeys,
    ...sideKeys(parentKeys).filter((key) => !ancestorKeySet.has(key)),
  ]
  ...
  const left = [
    ...ancestorKeys,
    ...allLeft.slice(ancestorKeys.length, ancestorKeys.length + VISIBLE_RELATIONS_PER_SIDE),
  ]
  const right = allRight.slice(0, VISIBLE_RELATIONS_PER_SIDE)
  const overflow = {
    parents: allLeft
      .slice(ancestorKeys.length + VISIBLE_RELATIONS_PER_SIDE)
      .map((key) => handles.get(key)!),
  ...
  const lineageKeys = [...ancestorKeys, focusKey]
  for (let index = 1; index < lineageKeys.length; index += 1) {
    edgePairs.add(`${lineageKeys[index - 1]}\u0000${lineageKeys[index]}`)
  }
