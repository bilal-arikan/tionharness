        ...
        <div className="absolute left-0 top-6 z-30 w-72 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-3 text-xs shadow-[var(--shadow-lg)]">
          ...
          {data &&
            ...
            (() => {
              ...
              const tokPerSec =
                data.durMs > 0 ? Math.round(data.outputTokens / (data.durMs / 1000)) : 0
              return (
...
// `title` carries the exact underlying value when `value` is a friendlier
// rendering of it (a model id shown as "Opus 5"), so the precise form a bug
// report needs stays one hover away.
function Row({
  label,
  value,
  mono,
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className={mono ? 'font-mono opacity-90' : 'font-medium'} title={title}>
        {value}
      </span>
    </div>
  )
