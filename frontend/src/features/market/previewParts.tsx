...
export function ColumnsPreview({ columns }: { columns?: { key: string; label: string; color?: string }[] }) {
...
export function ModelList({ models, providerId, prices }: { models?: string; providerId: string; prices: PriceTable }) {
...
export function StatChip({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: number }) {
  return (
...
export function MiniChip({ children }: { children: ReactNode }) {
  return (
...
export function SourceRefPreview({ url, items, loading }: { url: string; items: PreviewItem[] | null; loading: boolean }) {
  return (
