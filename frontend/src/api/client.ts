...
export function describeHttpError(status: number): string {
  switch (status) {
    ...
    case 503:
    case 504:
      return `Sunucuya ulaşılamıyor (HTTP ${status}). SwarmGo backend yanıt vermiyor — geliştirme sunucusunun (go run ./cmd/swarmgo, 127.0.0.1:8090) çalıştığından emin ol.`
    case 500:
