...
interface Props {
  /** Reveal the path in the OS file manager (usually an async API call). */
...
export function RevealButton({ onReveal, label, labelClassName = '', title = 'Klasörü aç', disabled }: Props) {
  ...
  return (
    <button type="button" onClick={click} disabled={disabled || busy} title={title} className={PATH_ACTION_CLS}>
